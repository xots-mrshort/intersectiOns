...
const RENDER_OVERLAP_OFFSET_PX = 7;
const RENDER_OVERLAP_NEAR_PX = 10;
const MAX_RENDER_LANE_SHIFT_PX = 8;
const ROUTE_SIDE_VARIETY_WEIGHT = 1.35;
const ROUTE_FAMILY_VARIETY_WEIGHT = 0.9;
const ROUTE_CROWDING_PENALTY_WEIGHT = 300;
const ROUTE_SHARED_CORRIDOR_PENALTY_WEIGHT = 900;
const ROUTE_SHARED_CORRIDOR_PERP_TOL_STEPS = 0.55;
const ROUTE_SHARED_CORRIDOR_MIN_OVERLAP_STEPS = 1.3;
const ROUTE_STATION_SIDE_VARIETY_WEIGHT = 3;
const STATION_APPROACH_DUPLICATE_PENALTY = 500;
const STATION_ANCHOR_NEARBY_RADIUS_STEPS = 1.0;
const STATION_JOIN_NODE_SIZE = 9;
const VISUAL_CORNER_RADIUS = 14;
let currentVisualOffsetLayout = null;
let drawnStationLabels = new Set();
...
function preload() {
    ...
    stopSmileySvg = loadImage('smiley circle.svg');
}
...
function setup () {
    ...
    joins = [];
    people = [];
    controlStations = panelStationNames.map(name => ({ name, people: [], anchor: null }));
    nextPersonId = 1;
    allSegments = [];
    lineColorOrder = shuffleArray(LINE_COLOR_PALETTE.slice());
    lineColorIndex = 0;
...
function draw () {
    background('#FBF4E1');
    drawnStationLabels = new Set();

    //drawGridVertices();
    drawHubs(); 
    //drawCentralBox();
    const renderOffsetLayout = buildLineRenderOffsetLayout();
    currentVisualOffsetLayout = renderOffsetLayout;
    updateVisualPaths(renderOffsetLayout);

    for (const person of people) {
        drawTrainLine(person);
    }

    drawJoins(renderOffsetLayout);
    drawEchoPulses();
    drawStops(renderOffsetLayout);
    drawStartPoints();
    drawTrainPulses();
...
function setupTrainCreatorControl() {
    const addButton = document.getElementById('addTrainBtn');
    const nameInput = document.getElementById('trainNameInput');
    const stationSelect = document.getElementById('trainStationSelect');
    if (!addButton || !nameInput || !stationSelect) return;

    const submit = () => {
        const name = nameInput.value.trim();
        const stationName = stationSelect.value;
        if (!name || !stationName) return;
        if (!findStationByName(stationName)) return;
        addPerson(name, stationName);
        nameInput.value = '';
        nameInput.focus();
    };
    ...
    syncTrainCreatorStations();
}

function syncTrainCreatorStations() {
    const addButton = document.getElementById('addTrainBtn');
    const stationSelect = document.getElementById('trainStationSelect');
    if (!addButton || !stationSelect) return;

    const previous = stationSelect.value;
    stationSelect.innerHTML = '';

    if (!controlStations.length) {
    ...
    const canRestore = controlStations.some(station => station.name === previous);
    stationSelect.value = canRestore ? previous : controlStations[0].name;
...
function renderStations() {
    ...
    for (const station of controlStations) {
        ...
        const stationRemoveButton = document.createElement('button');
        stationRemoveButton.type = 'button';
        stationRemoveButton.className = 'stationRemoveBtn';
        stationRemoveButton.textContent = 'Remove station';
        stationRemoveButton.addEventListener('click', () => {
            removeStation(station.name);
        });
        title.appendChild(stationRemoveButton);
        ...
        if (!station.people.length) {
            ...
            for (const personId of station.people) {
                ...
                removePersonButton.className = 'nameChipRemoveBtn';
                removePersonButton.textContent = 'x';
                removePersonButton.setAttribute('aria-label', `Remove ${person.name} from ${station.name}`);
                removePersonButton.addEventListener('click', () => {
                    removePersonFromStation(person.id, station.name);
                });
                chip.appendChild(removePersonButton);
...
function removePersonCompletely(personId) {
    const line = findLineByPersonId(personId);
    if (line) {
        lines = lines.filter(entry => entry.id !== line.id);
        allSegments = allSegments.filter(seg => seg.lineId !== line.id);
        lineGlowUntil.delete(line.id);
        echoPulses = echoPulses.filter(pulse => pulse.lineId !== line.id && pulse.sourceLineId !== line.id);
        intersectionBursts = intersectionBursts.filter(burst =>
            !Array.isArray(burst.lineIds) || !burst.lineIds.includes(line.id)
        );
    }

    people = people.filter(person => person.id !== personId);
    for (const station of controlStations) {
        if (!station || !Array.isArray(station.people)) continue;
        station.people = station.people.filter(id => id !== personId);
    }

    pulseStateByPerson.delete(personId);
    lastPulsePositionByPerson.delete(personId);
}

function removePersonFromStation(personId, stationName) {
    ...
    if (person && Array.isArray(person.stations)) {
        person.stations = person.stations.filter(name => name !== stationName);
        if (!person.stations.length && !person.isPrimary) {
            removePersonCompletely(person.id);
        } else if (!person.isPrimary) {
...
function removeStation(stationName) {
    const index = controlStations.findIndex(station => station.name === stationName);
    if (index < 0) return;

    const [removed] = controlStations.splice(index, 1);
    panelStationNames = panelStationNames.filter(name => name !== stationName);
    stationAnchorByName.delete(stationName);

    const removedNormalized = `${stationName}`.trim().toLowerCase();
    for (const [key, value] of intersectionNamesByKey.entries()) {
        if (`${value || ''}`.trim().toLowerCase() === removedNormalized) {
            intersectionNamesByKey.delete(key);
        }
    }

    const rerouteIds = new Set();
    for (const personId of removed.people || []) {
        const person = people.find(entry => entry.id === personId);
        if (!person || !Array.isArray(person.stations)) continue;
        person.stations = person.stations.filter(name => name !== stationName);
        if (!person.stations.length && !person.isPrimary) {
            removePersonCompletely(person.id);
            continue;
        }
        if (!person.isPrimary) rerouteIds.add(person.id);
    }

    for (const personId of rerouteIds) {
        const person = people.find(entry => entry.id === personId);
        if (person) regenerateLine(person);
    }
...
function getTrainSpeedForFate(fateValue) {
    const fate = constrain(round(fateValue), 1, 5);
    return mapValue(fate, 1, 5, 0.55, 1.45);
}
...
function updatePersonFate(personId, value) {
    ...
    person.speed = getTrainSpeedForFate(value);
...
function generateLineForPerson(person) {
    ...
    for (let attempt = 0; attempt < FATE_ROUTE_MAX_TRIES; attempt++) {
        ...
        candidate.speed = person.speed;
        candidate.lineColor = hexToRgbArray(person.color);
        candidate.routeFamily = spec.family || 'generic';
        candidate.routeStartSide = spec.startSide || getEdgeSideForPoint([start.x, start.y]);
        candidate.routeEndSide = spec.endSide || getEdgeSideForPoint([spec.end.x, spec.end.y]);
        candidate.run();
        if (!candidate.points || candidate.points.length < 2) continue;

        const stats = getCentralIntersectionStatsForLine(candidate);
        const fateScore = getFateFitScore(profile, stats);
        const stationScore = getStationAnchorFitScoreForPath(person, candidate.points, null);
        const crowdingPenalty = getPathCrowdingPenalty(candidate.points, null);
        const score = fateScore + stationScore + crowdingPenalty;
        if (score < bestScore) {
            bestScore = score;
...
function buildPerson(name, stationName, existingLine = null) {
    ...
    if (existingLine) {
        ...
        existingLine.speed = person.speed;
...
function drawTrainLine(person) {
    ...
    if (!line) return;
    const now = millis();
    const glowUntil = lineGlowUntil.get(line.id) || 0;
    const glowLevel = glowUntil > now ? constrain((glowUntil - now) / 900, 0, 1) : 0;
    const visualPath = Array.isArray(person.visualPath) && person.visualPath.length >= 2
        ? person.visualPath
        : person.path;
    ...
    if (glowLevel > 0) {
        ...
        noFill();
        drawRoundedPolyline(visualPath, 15);
    }
    stroke(person.color);
...
function drawVisualOffsetSegments(lineId, pathPoints, renderOffsetLayout = null) {
    ...
    for (let i = 0; i < segmentCount; i++) {
        ...
        const offset = getVisualSegmentOffset(lineId, i, renderOffsetLayout);
        const p1Offset = [p1[0] + offset[0], p1[1] + offset[1]];
        const p2Offset = [p2[0] + offset[0], p2[1] + offset[1]];
        const len = dist(p1Offset[0], p1Offset[1], p2Offset[0], p2Offset[1]);

        visualSegments.push({
            p1: p1Offset,
            p2: p2Offset,
            offset,
            laneGroup: getVisualSegmentLaneGroup(lineId, i, renderOffsetLayout),
            len
        });
    }

    const cornerLinks = new Array(pathPoints.length).fill(null);
    for (let vertexIndex = 1; vertexIndex < pathPoints.length - 1; vertexIndex++) {
        const prev = visualSegments[vertexIndex - 1];
        const next = visualSegments[vertexIndex];
        if (!prev || !next) continue;
        const prevHasOffset = abs(prev.offset[0]) > EPSILON || abs(prev.offset[1]) > EPSILON;
        const nextHasOffset = abs(next.offset[0]) > EPSILON || abs(next.offset[1]) > EPSILON;
        if (prevHasOffset || nextHasOffset) continue;
        if (prev.len < EPSILON || next.len < EPSILON) continue;

        const sharedCorner = [
            (prev.p2[0] + next.p1[0]) / 2,
            (prev.p2[1] + next.p1[1]) / 2
        ];
        const prevDir = [
            (prev.p2[0] - prev.p1[0]) / prev.len,
            (prev.p2[1] - prev.p1[1]) / prev.len
        ];
        const nextDir = [
            (next.p2[0] - next.p1[0]) / next.len,
            (next.p2[1] - next.p1[1]) / next.len
        ];
        const turnDot = prevDir[0] * nextDir[0] + prevDir[1] * nextDir[1];
        if (abs(turnDot) > 0.995) continue;

        const localRadius = min(
            VISUAL_CORNER_RADIUS,
            prev.len * 0.45,
            next.len * 0.45
        );
        if (localRadius < 0.5) continue;

        cornerLinks[vertexIndex] = {
            start: [
                sharedCorner[0] - prevDir[0] * localRadius,
                sharedCorner[1] - prevDir[1] * localRadius
            ],
            control: sharedCorner,
            end: [
                sharedCorner[0] + nextDir[0] * localRadius,
                sharedCorner[1] + nextDir[1] * localRadius
            ]
        };
    }
    ...
    for (let i = 0; i < visualSegments.length; i++) {
        ...
        let drawStart = seg.p1;
        let drawEnd = seg.p2;

        const startCorner = cornerLinks[i];
        if (startCorner) drawStart = startCorner.end;
        const endCorner = cornerLinks[i + 1];
        if (endCorner) drawEnd = endCorner.start;

        if (dist(drawStart[0], drawStart[1], drawEnd[0], drawEnd[1]) > EPSILON) {
            line(drawStart[0], drawStart[1], drawEnd[0], drawEnd[1]);
        }
    }

    for (let vertexIndex = 1; vertexIndex < cornerLinks.length - 1; vertexIndex++) {
        const corner = cornerLinks[vertexIndex];
        if (!corner) continue;
        beginShape();
        vertex(corner.start[0], corner.start[1]);
        quadraticVertex(
            corner.control[0],
            corner.control[1],
            corner.end[0],
...
function drawTrainPulses() {
    ...
    for (const person of people) {
        ...
        if (now >= pulse.pausedUntil) {
            const speed = max(0.1, person.speed || BASE_TRAIN_SPEED);
            pulse.t += pulse.direction * BASE_PULSE_STEP_PER_SEC * dtSec * speed;
            if (pulse.t >= 1) {
                pulse.t = 1;
                pulse.direction = -1;
            } else if (pulse.t <= 0) {
        ...
        const logicSample = samplePointOnPath(person.path, pulse.t);
        const visualPath = Array.isArray(person.visualPath) && person.visualPath.length >= 2
            ? person.visualPath
            : person.path;
        const visualSample = samplePointOnPath(visualPath, pulse.t);
        if (!logicSample || !visualSample) continue;
        ...
        handleIntersectionForPulse(person, pulse, logicSample.x + ox, logicSample.y + oy, pulseSize);
        ...
        push();
        translate(visualSample.x + ox, visualSample.y + oy);
        rotate(visualSample.angle || 0);
        imageMode(CENTER);
        tint(255, 210);
...
function handleIntersectionForPulse(person, pulse, x, y, pulseSize) {
    ...
    for (const stop of stops) {
        ...
        const triggerPoint = stop.lineTriggerPoints && stop.lineTriggerPoints[line.id]
            ? stop.lineTriggerPoints[line.id]
            : stop.point;

        const directHit = dist(x, y, triggerPoint[0], triggerPoint[1]) <= hitRadius;
        ...
        const stopKey = stop.key || `${round(stop.point[0])}:${round(stop.point[1])}`;
        ...
        intersectionBursts.push({
            x: stop.point[0],
            y: stop.point[1],
            stopPoint: [stop.point[0], stop.point[1]],
            lineIds: [waiting.lineId, line.id],
            startedAt: now,
            durationMs: 900,
...
function drawEchoPulses() {
    ...
    for (const echo of echoPulses) {
        ...
        const visualPath = Array.isArray(person.visualPath) && person.visualPath.length >= 2
            ? person.visualPath
            : person.path;
        const sample = samplePointOnPath(visualPath, echo.t);
        if (!sample) continue;
        ...
        if (!echo.arrived && progress >= 1 - EPSILON) {
            ...
            intersectionBursts.push({
                x: echo.stopPoint[0],
                y: echo.stopPoint[1],
                stopPoint: [echo.stopPoint[0], echo.stopPoint[1]],
                lineIds: [echo.lineId, echo.sourceLineId],
                startedAt: now,
                durationMs: 700,
...
function drawIntersectionBursts() {
    ...
    for (const burst of intersectionBursts) {
        ...
        let bx = burst.x;
        let by = burst.y;
        if (burst.stopPoint && Array.isArray(burst.lineIds) && burst.lineIds.length) {
            const visualPoints = burst.lineIds
                .map(lineId => getVisualPointForLineAtRawPoint(lineId, burst.stopPoint, currentVisualOffsetLayout))
                .filter(point => Array.isArray(point) && point.length === 2);
            if (visualPoints.length) {
                bx = visualPoints.reduce((sum, point) => sum + point[0], 0) / visualPoints.length;
                by = visualPoints.reduce((sum, point) => sum + point[1], 0) / visualPoints.length;
            }
        }
...
function projectPointToSegment(px, py, x1, y1, x2, y2) {
    ...
    return {
        ...
        x: projX,
        y: projY,
        distance: dist(px, py, projX, projY)
    };
}

function pointToSegmentDistance(px, py, x1, y1, x2, y2) {
    return projectPointToSegment(px, py, x1, y1, x2, y2).distance;
}

function getVisualSegmentOffset(lineId, segmentIndex, renderOffsetLayout = null) {
    const layout = renderOffsetLayout || currentVisualOffsetLayout;
    if (!layout || !layout.offsetBySegmentKey) return [0, 0];
    const key = `${lineId}:${segmentIndex}`;
    return layout.offsetBySegmentKey.get(key) || [0, 0];
}

function getVisualSegmentLaneGroup(lineId, segmentIndex, renderOffsetLayout = null) {
    const layout = renderOffsetLayout || currentVisualOffsetLayout;
    if (!layout || !layout.laneGroupBySegmentKey) return null;
    const key = `${lineId}:${segmentIndex}`;
    return layout.laneGroupBySegmentKey.get(key) ?? null;
}

function applyRenderedOffsetToPoint(layout, lineId, segmentIndex, point) {
    if (!point) return point;
    const offset = getVisualSegmentOffset(lineId, segmentIndex, layout);
    return [point[0] + offset[0], point[1] + offset[1]];
}

function buildVisualPathFromPath(lineId, pathPoints, renderOffsetLayout = null) {
    if (!pathPoints || pathPoints.length < 2) return pathPoints ? pathPoints.slice() : [];
    const segmentCount = pathPoints.length - 1;
    const runs = [];
    let runStart = 0;
    let runKind = classifySegment(pathPoints[0], pathPoints[1]);

    for (let segIndex = 1; segIndex < segmentCount; segIndex++) {
        const kind = classifySegment(pathPoints[segIndex], pathPoints[segIndex + 1]);
    ...
    runs.push({
        startSeg: runStart,
        endSeg: segmentCount - 1,
        kind: runKind,
        length: segmentCount - runStart
    });

    for (const run of runs) {
        const offset = getVisualSegmentOffset(lineId, run.startSeg, renderOffsetLayout);
        run.offset = [offset[0], offset[1]];
    }

    const pointOwners = new Array(pathPoints.length).fill(null).map(() => []);
    for (const run of runs) {
        for (let pointIndex = run.startSeg; pointIndex <= run.endSeg + 1; pointIndex++) {
            pointOwners[pointIndex].push(run);
        }
    }
    ...
    for (let i = 0; i < pathPoints.length; i++) {
        ...
        const owners = pointOwners[i];
        if (!point || !owners.length) {
            shifted.push(point ? [point[0], point[1]] : [0, 0]);
            continue;
        }

        let chosen = owners[0];
        for (const candidate of owners) {
            const chosenInterior = i > chosen.startSeg && i < chosen.endSeg + 1;
            const candidateInterior = i > candidate.startSeg && i < candidate.endSeg + 1;
            if (candidateInterior && !chosenInterior) {
                chosen = candidate;
                continue;
            }
            if (candidateInterior === chosenInterior && candidate.length > chosen.length) {
                chosen = candidate;
            }
        }

        shifted.push([
            point[0] + chosen.offset[0],
            point[1] + chosen.offset[1]
        ]);
    }
...
function updateVisualPaths(renderOffsetLayout = null) {
    for (const person of people) {
        if (!person || !Array.isArray(person.path) || person.path.length < 2) {
            if (person) person.visualPath = Array.isArray(person.path) ? person.path.slice() : [];
            continue;
        }
        const line = findLineByPersonId(person.id);
        if (!line) {
            person.visualPath = person.path.map(point => [point[0], point[1]]);
            continue;
        }
        person.visualPath = buildVisualPathFromPath(line.id, person.path, renderOffsetLayout);
    }
}

function getVisualPointForLineAtRawPoint(lineId, rawPoint, renderOffsetLayout = null) {
    const line = lines.find(entry => entry.id === lineId);
    if (!line || !rawPoint) return rawPoint;
    const closest = findClosestPointOnLineToAnchor(line, rawPoint);
    if (!closest) return rawPoint;
    const visual = applyRenderedOffsetToPoint(renderOffsetLayout, lineId, closest.segmentIndex, closest.point);
    return visual || rawPoint;
}

function getSegmentProjectionRange(seg) {
    ...
    if (seg.kind === 'diagDown') {
        const a = seg.a[0] + seg.a[1];
        const b = seg.b[0] + seg.b[1];
...
function getPerpendicularDistanceForSameKind(segA, segB) {
    if (segA.kind !== segB.kind) return Infinity;
    if (segA.kind === 'horizontal') return abs(segA.a[1] - segB.a[1]);
    if (segA.kind === 'vertical') return abs(segA.a[0] - segB.a[0]);
    if (segA.kind === 'diagDown') return abs((segA.a[1] - segA.a[0]) - (segB.a[1] - segB.a[0])) * 0.70710678118;
    return abs((segA.a[1] + segA.a[0]) - (segB.a[1] + segB.a[0])) * 0.70710678118;
}

function overlapAmount(rangeA, rangeB) {
    return min(rangeA[1], rangeB[1]) - max(rangeA[0], rangeB[0]);
}

function isNearlySharedSegment(segA, segB, nearPx, minOverlapPx) {
    if (segA.lineId === segB.lineId) return false;
    if (segA.kind !== segB.kind) return false;
    const perpDist = getPerpendicularDistanceForSameKind(segA, segB);
    if (perpDist > nearPx) return false;
    const projA = getSegmentProjectionRange(segA);
    const projB = getSegmentProjectionRange(segB);
    return overlapAmount(projA, projB) >= minOverlapPx;
}

function getCanonicalNormalByKind(kind) {
    if (kind === 'horizontal') return [0, 1];
    if (kind === 'vertical') return [1, 0];
    if (kind === 'diagDown') return [0.70710678118, -0.70710678118];
...
function getSegmentCorridorSignature(seg) {
    if (seg.kind === 'horizontal') return `h:${signedCoord(seg.a[1])}`;
    if (seg.kind === 'vertical') return `v:${signedCoord(seg.a[0])}`;
    if (seg.kind === 'diagDown') return `d1:${signedCoord(seg.a[1] - seg.a[0])}`;
    return `d2:${signedCoord(seg.a[1] + seg.a[0])}`;
}

function buildLineCorridorRuns(segments) {
    const runs = [];
    const segmentsByLine = new Map();

    for (const seg of segments) {
        if (!segmentsByLine.has(seg.lineId)) segmentsByLine.set(seg.lineId, []);
        segmentsByLine.get(seg.lineId).push(seg);
    }

    for (const [lineId, lineSegments] of segmentsByLine.entries()) {
        if (!lineSegments.length) continue;
        let currentRun = null;

        for (const seg of lineSegments) {
            const signature = getSegmentCorridorSignature(seg);
            const canExtend =
                currentRun &&
                currentRun.kind === seg.kind &&
                currentRun.signature === signature;

            if (canExtend) {
                currentRun.segmentIndexes.push(seg.segmentIndex);
                currentRun.b = seg.b;
                continue;
            }

            currentRun = {
                key: `${lineId}:${seg.segmentIndex}`,
                lineId,
                kind: seg.kind,
                signature,
                a: seg.a,
                b: seg.b,
                segmentIndexes: [seg.segmentIndex]
            };
...
function buildLaneShiftByLineId(lineIds) {
    const shifts = new Map();
    if (!lineIds || !lineIds.length) return shifts;
    if (lineIds.length === 1) {
        shifts.set(lineIds[0], 0);
        return shifts;
    }

    const center = (lineIds.length - 1) / 2;
    const maxAbs = max(0.0001, center);
    for (let i = 0; i < lineIds.length; i++) {
        const raw = i - center;
        const normalized = raw / maxAbs;
        // Non-linear spacing: spread middle lanes farther apart to avoid visual stacking.
        const curved = (normalized < 0 ? -1 : 1) * pow(abs(normalized), 0.68);
        const laneShift = constrain(
            curved * RENDER_OVERLAP_OFFSET_PX,
            -MAX_RENDER_LANE_SHIFT_PX,
            MAX_RENDER_LANE_SHIFT_PX
        );
        shifts.set(lineIds[i], laneShift);
    }

    return shifts;
}

function buildLineRenderOffsetLayout() {
    const segments = [];
    ...
    for (const line of lines) {
        ...
        for (let i = 0; i < line.points.length - 1; i++) {
            ...
            if (!a || !b) continue;
            if (dist(a[0], a[1], b[0], b[1]) < EPSILON) continue;
            segments.push({
                key: `${line.id}:${i}`,
                lineId: line.id,
                segmentIndex: i,
                a,
                b,
                kind: classifySegment(a, b)
            });
        }
    }

    const runs = buildLineCorridorRuns(segments);
    if (runs.length < 2) {
        return { offsetBySegmentKey: new Map(), laneGroupBySegmentKey: new Map() };
    }

    const parent = runs.map((_, idx) => idx);
    const findRoot = (idx) => {
        let root = idx;
    ...
    const minOverlapPx = max(8, distLength * 0.35);
    for (let i = 0; i < runs.length - 1; i++) {
        for (let j = i + 1; j < runs.length; j++) {
            if (!isNearlySharedSegment(runs[i], runs[j], RENDER_OVERLAP_NEAR_PX, minOverlapPx)) continue;
            union(i, j);
        }
    }
    ...
    const offsetBySegmentKey = new Map();
    const laneGroupBySegmentKey = new Map();
    const lineOffsetAccumulator = new Map();
    let laneGroupId = 0;
    for (const memberIndexes of groups.values()) {
        const lineIds = Array.from(new Set(memberIndexes.map(idx => runs[idx].lineId))).sort((a, b) => a - b);
        if (lineIds.length < 2) continue;
        const groupId = laneGroupId++;
        const lineShiftById = buildLaneShiftByLineId(lineIds);

        for (const runIndex of memberIndexes) {
            const run = runs[runIndex];
            for (const segmentIndex of run.segmentIndexes) {
                const segKey = `${run.lineId}:${segmentIndex}`;
                laneGroupBySegmentKey.set(segKey, groupId);
            }

            const laneShift = lineShiftById.get(run.lineId) || 0;
            if (abs(laneShift) < EPSILON) continue;
            const normal = getCanonicalNormalByKind(run.kind);
            const runOffset = [
                normal[0] * laneShift,
                normal[1] * laneShift
            ];
            const weight = max(1, run.segmentIndexes.length);
            const acc = lineOffsetAccumulator.get(run.lineId) || { x: 0, y: 0, w: 0 };
            acc.x += runOffset[0] * weight;
            acc.y += runOffset[1] * weight;
            acc.w += weight;
            lineOffsetAccumulator.set(run.lineId, acc);
        }
    }

    const lineOffsetByLineId = new Map();
    for (const [lineId, acc] of lineOffsetAccumulator.entries()) {
        if (!acc || acc.w <= 0) continue;
        let ox = acc.x / acc.w;
        let oy = acc.y / acc.w;
        const mag = sqrt(ox * ox + oy * oy);
        if (mag > MAX_RENDER_LANE_SHIFT_PX) {
            const scale = MAX_RENDER_LANE_SHIFT_PX / mag;
            ox *= scale;
            oy *= scale;
        }
        if (abs(ox) < 0.2) ox = 0;
        if (abs(oy) < 0.2) oy = 0;
        lineOffsetByLineId.set(lineId, [ox, oy]);
    }

    for (const seg of segments) {
        const offset = lineOffsetByLineId.get(seg.lineId);
        if (!offset) continue;
        if (abs(offset[0]) < EPSILON && abs(offset[1]) < EPSILON) continue;
        offsetBySegmentKey.set(seg.key, [offset[0], offset[1]]);
    }

    return { offsetBySegmentKey, laneGroupBySegmentKey };
}

function findClosestPointOnLineToAnchor(line, anchor) {
    if (!line || !line.points || line.points.length < 2 || !anchor) return null;
    let best = null;

    for (let i = 0; i < line.points.length - 1; i++) {
        const a = line.points[i];
        const b = line.points[i + 1];
        const projected = projectPointToSegment(anchor[0], anchor[1], a[0], a[1], b[0], b[1]);
...
function pickJoinCluster(points, radius, anchor) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        while (queue.length) {
            ...
            for (let j = 0; j < points.length; j++) {
                ...
                const dx = points[current].renderedPoint[0] - points[j].renderedPoint[0];
                const dy = points[current].renderedPoint[1] - points[j].renderedPoint[1];
    ...
    clusters.sort((a, b) => {
        ...
        const aMean = a.reduce((sum, item) => sum + dist(item.renderedPoint[0], item.renderedPoint[1], anchor[0], anchor[1]), 0) / a.length;
        const bMean = b.reduce((sum, item) => sum + dist(item.renderedPoint[0], item.renderedPoint[1], anchor[0], anchor[1]), 0) / b.length;
        return aMean - bMean;
    });
...
function drawJoins(renderOffsetLayout = null) {
    if (!controlStations || !controlStations.length) return;

    for (const station of controlStations) {
        if (!station || !Array.isArray(station.people) || station.people.length < 2) continue;
        ...
        for (const personId of station.people) {
            const person = people.find(entry => entry.id === personId);
            if (!person) continue;
            const personLine = findLineByPersonId(person.id);
            if (!personLine || !personLine.points || personLine.points.length < 2) continue;

            const closest = findClosestPointOnLineToAnchor(personLine, anchor);
            if (!closest) continue;

            const rendered = applyRenderedOffsetToPoint(
                renderOffsetLayout,
                personLine.id,
            ...
            joinCandidates.push({
                lineId: personLine.id,
                personId: person.id,
                distance: closest.distance,
                renderedPoint: rendered || closest.point
            });
        }

        if (joinCandidates.length < 2) continue;

        push();
        stroke(38, 34, 29, 185);
        strokeWeight(2.2);
        for (const item of joinCandidates) {
            line(item.renderedPoint[0], item.renderedPoint[1], anchor[0], anchor[1]);
        }
        pop();
        ...
        stroke(38, 34, 29, 220);
        strokeWeight(2);
        circle(anchor[0], anchor[1], STATION_JOIN_NODE_SIZE + 3);
        pop();
        ...
        textSize(13);
        text(station.name, anchor[0] + 10, anchor[1] - 12);
        pop();
        drawnStationLabels.add((station.name || '').trim().toLowerCase());
    }
}

function clusterStopDisplayPoints(items, radiusPx) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        while (queue.length) {
            ...
            const a = items[idx].point;
            for (let j = 0; j < items.length; j++) {
                if (visited.has(j)) continue;
                const b = items[j].point;
                const dx = a[0] - b[0];
                const dy = a[1] - b[1];
                if (dx * dx + dy * dy > radiusSq) continue;
                visited.add(j);
        ...
        for (const member of members) {
            center[0] += member.point[0];
            center[1] += member.point[1];
        }
...
function weightedPickByWeight(items, getWeight) {
    if (!Array.isArray(items) || !items.length) return null;
    const weighted = [];
    let total = 0;

    for (const item of items) {
        const w = max(0.0001, getWeight(item) || 0);
        weighted.push({ item, w });
        total += w;
    }
    if (total <= EPSILON) return random(items);
...
function getEdgeSideForPoint(point) {
    if (!point || !gridBounds) return 'ANY';
    const x = point[0];
    const y = point[1];
    const dW = abs(x - gridBounds.minX);
    const dE = abs(x - gridBounds.maxX);
    const dN = abs(y - gridBounds.minY);
    const dS = abs(y - gridBounds.maxY);
    const minD = min(dW, dE, dN, dS);
    const tol = distLength * 0.55;

    const atW = dW <= minD + EPSILON && dW <= tol;
    const atE = dE <= minD + EPSILON && dE <= tol;
    const atN = dN <= minD + EPSILON && dN <= tol;
    const atS = dS <= minD + EPSILON && dS <= tol;

    if ((atW || atE) && (atN || atS)) {
        if (atW && atN) return 'NW';
        if (atE && atN) return 'NE';
        if (atW && atS) return 'SW';
        if (atE && atS) return 'SE';
    }
...
function getRouteDiversityUsage() {
    const sideUsage = new Map();
    const familyUsage = new Map();

    for (const line of lines) {
        if (!line || !Array.isArray(line.points) || !line.points.length) continue;
        const side = line.routeStartSide || getEdgeSideForPoint(line.points[0]);
        const family = line.routeFamily || 'generic';
        sideUsage.set(side, (sideUsage.get(side) || 0) + 1);
        familyUsage.set(family, (familyUsage.get(family) || 0) + 1);
    }

    return { sideUsage, familyUsage };
}

function getStationStartSideUsageForPerson(person) {
    const usage = new Map();
    if (!person || !Array.isArray(person.stations)) return usage;
    ...
    for (const stationName of person.stations) {
        ...
        for (const personId of station.people) {
            ...
            const line = findLineByPersonId(personId);
            if (!line) continue;
            const side = line.routeStartSide || getEdgeSideForPoint(line.points && line.points[0]);
            if (!side) continue;
...
function pickRouteSpecForPerson(person) {
    ...
    const { sideUsage, familyUsage } = getRouteDiversityUsage();
    const stationStartUsage = getStationStartSideUsageForPerson(person);
    const candidates = person.fate >= 4
        ? pool.filter(spec => spec.secondaryHub || spec.family === 'centerBox' || random() < 0.5)
        : pool;
    const chooseFrom = candidates.length ? candidates : pool;

    return weightedPickByWeight(chooseFrom, (spec) => {
        const sideCount = sideUsage.get(spec.startSide || 'ANY') || 0;
        const familyCount = familyUsage.get(spec.family || 'generic') || 0;
        const stationSideCount = stationStartUsage.get(spec.startSide || 'ANY') || 0;
        let weight = 1;
        weight /= (1 + sideCount * ROUTE_SIDE_VARIETY_WEIGHT);
        weight /= (1 + familyCount * ROUTE_FAMILY_VARIETY_WEIGHT);
        weight /= (1 + stationSideCount * ROUTE_STATION_SIDE_VARIETY_WEIGHT);
        if (person && person.fate >= 4 && (spec.secondaryHub || spec.family === 'centerBox')) {
            weight *= 1.35;
        }
        weight *= random(0.88, 1.18);
        return weight;
    }) || random(chooseFrom);
}
...
function pickStationAnchorFromPrimary(stationName) {
    ...
    const sampled = samplePointOnPath(primaryLine.points, ratio);
...
function getOrCreateStationAnchor(stationName) {
    const station = findStationByName(stationName);
    if (station && Array.isArray(station.anchor) && station.anchor.length === 2) {
        stationAnchorByName.set(stationName, [station.anchor[0], station.anchor[1]]);
        return station.anchor;
    }
    const cached = stationAnchorByName.get(stationName);
    if (cached) {
        if (station) station.anchor = [cached[0], cached[1]];
        return cached;
    }
    const anchor = pickStationAnchorFromPrimary(stationName);
    if (!anchor) return null;
    stationAnchorByName.set(stationName, anchor);
    if (station) station.anchor = [anchor[0], anchor[1]];
    return anchor;
}

function pathTouchesPoint(pathPoints, point, tolerance = 0.5) {
    if (!Array.isArray(pathPoints) || pathPoints.length < 2 || !point) return false;

    for (const p of pathPoints) {
        if (dist(p[0], p[1], point[0], point[1]) <= tolerance) return true;
    }

    for (let i = 0; i < pathPoints.length - 1; i++) {
        const a = pathPoints[i];
        const b = pathPoints[i + 1];
        if (pointToSegmentDistance(point[0], point[1], a[0], a[1], b[0], b[1]) <= tolerance) {
            return true;
        }
    }
    return false;
...
function classifyDirection8(dx, dy) {
    const sx = abs(dx) < EPSILON ? 0 : (dx > 0 ? 1 : -1);
    const sy = abs(dy) < EPSILON ? 0 : (dy > 0 ? 1 : -1);
...
function getStationAnchorNearbyRadiusPx() {
    return distLength * STATION_ANCHOR_NEARBY_RADIUS_STEPS;
}

function getPathApproachSideAtAnchor(pathPoints, anchor, nearRadiusPx = 0.75) {
    if (!Array.isArray(pathPoints) || pathPoints.length < 2 || !anchor) return null;

    for (let i = 0; i < pathPoints.length; i++) {
        const p = pathPoints[i];
        if (!pointsEqual(p, anchor)) continue;

        if (i > 0) {
            const prev = pathPoints[i - 1];
            return classifyDirection8(prev[0] - anchor[0], prev[1] - anchor[1]);
        }
        if (i < pathPoints.length - 1) {
            const next = pathPoints[i + 1];
            return classifyDirection8(next[0] - anchor[0], next[1] - anchor[1]);
        }
    }
    ...
    for (let i = 0; i < pathPoints.length - 1; i++) {
        ...
        const projected = projectPointToSegment(anchor[0], anchor[1], a[0], a[1], b[0], b[1]);
        if (!best || projected.distance < best.distance) {
            best = { distance: projected.distance, a, b };
        }
    }
    if (!best || best.distance > nearRadiusPx) return null;
    const da = dist(anchor[0], anchor[1], best.a[0], best.a[1]);
    const db = dist(anchor[0], anchor[1], best.b[0], best.b[1]);
    const ref = da >= db ? best.a : best.b;
    return classifyDirection8(ref[0] - anchor[0], ref[1] - anchor[1]);
}

function getStationApproachUsage(stationName, anchor, ignoreLineId = null) {
    const usage = new Map();
    const station = findStationByName(stationName);
    if (!station || !Array.isArray(station.people) || !anchor) return usage;
    const nearRadiusPx = getStationAnchorNearbyRadiusPx();

    for (const personId of station.people) {
        const line = findLineByPersonId(personId);
        if (!line || !Array.isArray(line.points) || line.points.length < 2) continue;
        if (ignoreLineId !== null && line.id === ignoreLineId) continue;
        if (closestDistancePathToPoint(line.points, anchor) > nearRadiusPx) continue;

        const side = getPathApproachSideAtAnchor(line.points, anchor, nearRadiusPx);
...
function closestDistancePathToPoint(pathPoints, point) {
    if (!Array.isArray(pathPoints) || pathPoints.length < 2 || !point) return Infinity;
    let best = Infinity;
    for (let i = 0; i < pathPoints.length - 1; i++) {
        const a = pathPoints[i];
        const b = pathPoints[i + 1];
...
function getStationAnchorFitScoreForPath(person, pathPoints, ignoreLineId = null) {
    if (!person || !Array.isArray(person.stations) || !person.stations.length) return 0;
    if (!Array.isArray(pathPoints) || pathPoints.length < 2) return 2000;
    ...
    for (const stationName of person.stations) {
        ...
        const nearest = closestDistancePathToPoint(pathPoints, anchor);
        if (nearest > nearRadiusPx) {
            // Strong pull toward station hubs so lines converge there.
            const overflow = nearest - nearRadiusPx;
            const overflowSteps = overflow / max(1, distLength);
            score += 600 + overflowSteps * 220 + overflowSteps * overflowSteps * 280;
            continue;
        }

        const candidateSide = getPathApproachSideAtAnchor(pathPoints, anchor, nearRadiusPx);
        if (!candidateSide || candidateSide === 'C') continue;
        const approachUsage = getStationApproachUsage(stationName, anchor, ignoreLineId);
        const duplicates = approachUsage.get(candidateSide) || 0;
        score += duplicates * STATION_APPROACH_DUPLICATE_PENALTY;
    }

    return score;
}

function getPathCrowdingPenalty(pathPoints, ignoreLineId = null) {
    if (!Array.isArray(pathPoints) || pathPoints.length < 2) return 0;
    if (!Array.isArray(lines) || !lines.length) return 0;

    const sampleStep = max(1, floor(pathPoints.length / 22));
    const hardOverlapDist = distLength * 0.14;
    const softOverlapDist = distLength * 0.34;
    const corridorPerpTol = distLength * ROUTE_SHARED_CORRIDOR_PERP_TOL_STEPS;
    const corridorMinOverlap = distLength * ROUTE_SHARED_CORRIDOR_MIN_OVERLAP_STEPS;
    ...
    for (const existing of lines) {
        if (!existing || !Array.isArray(existing.points) || existing.points.length < 2) continue;
        if (ignoreLineId !== null && existing.id === ignoreLineId) continue;

        let overlapScore = 0;
        let sampleCount = 0;

        for (let i = 0; i < pathPoints.length; i += sampleStep) {
            const p = pathPoints[i];
            sampleCount++;
            let nearest = Infinity;

            for (let s = 0; s < existing.points.length - 1; s++) {
                const a = existing.points[s];
                const b = existing.points[s + 1];
                const d = pointToSegmentDistance(p[0], p[1], a[0], a[1], b[0], b[1]);
                if (d < nearest) nearest = d;
                if (nearest <= hardOverlapDist) break;
            }

            if (nearest <= hardOverlapDist) {
                overlapScore += 1;
            } else if (nearest <= softOverlapDist) {
                overlapScore += 0.35;
            }
        }

        if (sampleCount > 0) {
            const overlapRatio = overlapScore / sampleCount;
            penalty += overlapRatio * ROUTE_CROWDING_PENALTY_WEIGHT;
        }

        // Extra penalty when candidate follows the same corridor for a long stretch.
        let sharedCorridorSteps = 0;
        for (const candSeg of candidateSegments) {
            ...
            for (let s = 0; s < existing.points.length - 1; s++) {
                ...
                if (candSeg.kind !== existSeg.kind) continue;
                if (getPerpendicularDistanceForSameKind(candSeg, existSeg) > corridorPerpTol) continue;
                const overlapPx = overlapAmount(
                    getSegmentProjectionRange(candSeg),
                    getSegmentProjectionRange(existSeg)
                );
                if (overlapPx > bestOverlapPx) bestOverlapPx = overlapPx;
            }
            if (bestOverlapPx >= corridorMinOverlap) {
                sharedCorridorSteps += bestOverlapPx / max(1, distLength);
            }
        }
        if (sharedCorridorSteps > 0) {
            penalty += sharedCorridorSteps * ROUTE_SHARED_CORRIDOR_PENALTY_WEIGHT;
        }
    }
...
function rerouteLineForFateProfile(person, line) {
    ...
    for (let i = 0; i < FATE_ROUTE_MAX_TRIES; i++) {
        ...
        const fateScore = getFateFitScore(profile, stats);
        const stationScore = getStationAnchorFitScoreForPath(person, line.points, line.id);
        const crowdingPenalty = getPathCrowdingPenalty(line.points, line.id);
        const score = fateScore + stationScore + crowdingPenalty;
        if (score < bestScore) {
            bestScore = score;
...
function mousePressed() {
    ...
    if (!trimmed) {
        ...
        const stationName = upsertStationFromStopName(trimmed, hit.point);
        syncStationPeopleFromIntersectingLines(stationName, hit.lines || []);
        intersectionNamesByKey.set(key, stationName);
        hit.name = stationName;
        renderStations();
        renderTrains();
    }
    redraw();
...
function upsertStationFromStopName(name, point) {
    const stationName = `${name || ''}`.trim();
    if (!stationName) return '';

    const normalized = stationName.toLowerCase();
    const existing = controlStations.find(station =>
        `${station && station.name ? station.name : ''}`.trim().toLowerCase() === normalized
    );

    const anchor = Array.isArray(point) && point.length === 2
        ? [point[0], point[1]]
        : null;
    ...
    if (existing) {
        if (anchor) {
            ...
            stationAnchorByName.set(existing.name, anchor);
        }
...
function syncStationPeopleFromIntersectingLines(stationName, lineIds) {
    const station = findStationByName(stationName);
    if (!station || !Array.isArray(lineIds) || !lineIds.length) return;
    if (!Array.isArray(station.people)) station.people = [];

    const uniqueLineIds = Array.from(new Set(lineIds));
    for (const lineId of uniqueLineIds) {
        const person = findPersonByLineId(lineId);
        if (!person) continue;

        if (!station.people.includes(person.id)) {
            station.people.push(person.id);
        }

        if (!Array.isArray(person.stations)) {
            person.stations = [stationName];
        } else if (!person.stations.includes(stationName)) {
...
function buildClusteredLogicStopsFromMap(stopMap) {
    const rawStops = Array.from(stopMap.values()).filter(stop => stop.lines && stop.lines.size >= 2);
    if (!rawStops.length) return [];

    const clusterRadius = max(10, distLength * STATION_ANCHOR_NEARBY_RADIUS_STEPS);
    ...
    for (let i = 0; i < rawStops.length; i++) {
        ...
        while (queue.length) {
            ...
            for (let j = 0; j < rawStops.length; j++) {
                ...
                if (dx * dx + dy * dy > clusterRadiusSq) continue;
                visited.add(j);
                queue.push(j);
            }
        }
    ...
    for (let clusterIndex = 0; clusterIndex < clusters.length; clusterIndex++) {
        ...
        for (const member of members) {
            center[0] += member.point[0];
            center[1] += member.point[1];
            for (const lineId of member.lines) lineSet.add(lineId);
        }
        center[0] /= members.length;
        center[1] /= members.length;
        if (lineSet.size < 2) continue;

        const lineTriggerPoints = {};
        for (const lineId of lineSet) {
            const line = lines.find(entry => entry.id === lineId);
            if (!line || !Array.isArray(line.points) || line.points.length < 2) continue;
            const closest = findClosestPointOnLineToAnchor(line, center);
            if (!closest) continue;
            lineTriggerPoints[lineId] = [closest.point[0], closest.point[1]];
        }

        let pickedName = '';
        for (const member of members) {
            const named = intersectionNamesByKey.get(stopKeyFromPoint(member.point[0], member.point[1]));
            if (named) {
                pickedName = named;
                break;
            }
        ...
        logicStops.push({
            key: `cluster:${round(center[0])}:${round(center[1])}:${clusterIndex}`,
...
function addStops() {
    ...
    appendStationAnchorStops(stopMap);
    stops = buildClusteredLogicStopsFromMap(stopMap);
...
function drawStops(renderOffsetLayout = null) {
    ...
    for (const s of stops) {
        const visualPoints = (s.lines || [])
            .map(lineId => getVisualPointForLineAtRawPoint(lineId, s.point, renderOffsetLayout))
            .filter(point => Array.isArray(point) && point.length === 2);
        const representative = visualPoints.length
            ? [
                visualPoints.reduce((sum, point) => sum + point[0], 0) / visualPoints.length,
                visualPoints.reduce((sum, point) => sum + point[1], 0) / visualPoints.length
            ]
            : [s.point[0], s.point[1]];
        displayStops.push({
    ...
    const stopClusters = clusterStopDisplayPoints(displayStops, max(10, distLength * 0.6));
    for (const cluster of stopClusters) {
        const point = cluster.center;
        push();
        imageMode(CENTER);
        image(stopSmileySvg, point[0], point[1], drawWidth, drawHeight);
        const normalizedName = (cluster.name || '').trim().toLowerCase();
        if (cluster.name && !drawnStationLabels.has(normalizedName)) {
            noStroke();
            fill(38, 34, 29, 210);
            textAlign(LEFT, CENTER);
            textSize(13);
            text(cluster.name, point[0] + 10, point[1] - 12);
            drawnStationLabels.add(normalizedName);
        }
        pop();
    }
}
...
function buildRouteFamilySpecs() {
    ...
    const makeSpec = (startSide, hub, extra = {}) => {
        ...
        return {
            ...
            startSide,
            endSide,
            family: extra.family || 'generic',
            ...extra
        };
    };

    for (let i = 0; i < FAMILY_COUNTS.westEast; i++) {
        const startSide = random(['W', 'E']);
        specs.push(makeSpec(startSide, pickDistinctHub('horizontal', usedHubKeys), { family: 'westEast' }));
    }

    for (let i = 0; i < FAMILY_COUNTS.northSouth; i++) {
        const startSide = random(['N', 'S']);
        specs.push(makeSpec(startSide, pickDistinctHub('vertical', usedHubKeys), { family: 'northSouth' }));
    }

    for (let i = 0; i < FAMILY_COUNTS.swNe; i++) {
        const startSide = random(['SW', 'NE']);
        specs.push(makeSpec(startSide, pickDistinctHub('diagUp', usedHubKeys), { family: 'swNe' }));
    }

    for (let i = 0; i < FAMILY_COUNTS.nwSe; i++) {
        const startSide = random(['NW', 'SE']);
        specs.push(makeSpec(startSide, pickDistinctHub('diagDown', usedHubKeys), { family: 'nwSe' }));
    }

    for (let i = 0; i < FAMILY_COUNTS.shortBranch; i++) {
        const sideA = random(['W', 'N', 'E', 'S', 'NW', 'NE', 'SW', 'SE']);
        const branchEnd = nextClockwiseSide(sideA);
        specs.push(makeSpec(sideA, pickDistinctHub('inner', usedHubKeys), { endSide: branchEnd, family: 'shortBranch' }));
    }

    for (let i = 0; i < FAMILY_COUNTS.centerBox; i++) {
        const sideA = random(['W', 'E', 'N', 'S']);
        specs.push(makeSpec(sideA, pickDistinctHub('top', usedHubKeys), {
            secondaryHub: pickDistinctHub('bottom', usedHubKeys),
            family: 'centerBox'
        }));
    }
...
function nextClockwiseSide(side) {
    ...
    if (side === 'E') return 'S';
    if (side === 'S') return 'W';
    if (side === 'NW') return 'NE';
    if (side === 'NE') return 'SE';
    if (side === 'SE') return 'SW';
    if (side === 'SW') return 'NW';
    return 'W';
}
